...
from compras.models import EstadoCompra, OrdenCompra, Proveedor
from compras.services import CompraService
from core.models import EstadoPago
from inventario.models import Stock, Movimiento, TipoMovimiento
...
from ventas.models import Orden, OrdenItem, TipoEntrega
...
def crear_almacen():
    ubicacion = Ubicacion.objects.create(pais='Colombia', ciudad='Medellín')
    return Almacen.objects.create(nombre='Bodega Test', ubicacion=ubicacion)
...
def crear_tipo_movimiento_entrada():
    tipo, _ = TipoMovimiento.objects.get_or_create(id=1, defaults={'nombre': 'Entrada'})
    return tipo


def crear_tipo_movimiento_salida():
    tipo, _ = TipoMovimiento.objects.get_or_create(id=2, defaults={'nombre': 'Salida'})
    return tipo
...
class OrdenVentaServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.tipo_entrega = TipoEntrega.objects.create(nombre='En tienda')
        self.tipo_movimiento_salida = crear_tipo_movimiento_salida()
        crear_tipo_movimiento_entrada()
        ...
        tipo_cliente = TipoCliente.objects.create(nombre='Regular')
...
class PagoServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.tipo_entrega = TipoEntrega.objects.create(nombre='Domicilio')
        crear_tipo_movimiento_salida()

        self.pendiente, self.abonado, self.completado = crear_estados_pago()
        crear_stock(self.producto, self.almacen, 20)

        tipo_cliente = TipoCliente.objects.create(nombre='Regular')
        self.cliente = Cliente.objects.create(nombre='Cliente', tipo_cliente=tipo_cliente)
...
class CompraServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.estado_pendiente = EstadoCompra.objects.create(nombre='Pendiente')
        self.estado_recibido = EstadoCompra.objects.create(nombre='Recibido')
