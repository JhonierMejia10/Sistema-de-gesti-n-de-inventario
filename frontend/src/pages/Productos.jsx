...
export default function Productos() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [prodRes, catRes, marcasRes, tiposRes, almacenesRes] = await Promise.all([
                api.get('/api/v1/productos/'),
                api.get('/api/v1/categorias/'),
                api.get('/api/v1/marcas/'),
                api.get('/api/v1/tipos-de-producto/'),
            ...
            console.error('Error fetching productos dependencies:', error);
                                    ...
                                    <tr key={i} className="hover:bg-[#1a1a1a] subtle-transition group">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-[#a1a1aa] text-xs mt-0.5">
                                                {item.marca
                                                    ? (typeof item.marca === 'object' ? item.marca.nombre : item.marca)
                                                    : 'Sin Marca'}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 font-bold text-[#10b981] text-right">${item.precio || item.price || '0.00'}</td>
