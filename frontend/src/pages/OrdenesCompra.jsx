...
import { Eye, Plus, Trash2, ShoppingCart, PlusCircle } from 'lucide-react';
    ...
    const fetchData = async () => {
        ...
        try {
            const [
                ...
                api.get('/api/v1/estados-de-compra/')
            ]);
            ...
            setEstadosCompra(estadoCRes.data.results || estadoCRes.data || []);

            // Mapear productos para resolución de nombres
            const pMap = {};
            prods.forEach(p => {
            ...
            console.error('Error fetching compras o dependencias:', error);
                        ...
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr className="border-b border-[#27272a] text-[#a1a1aa] font-medium">
                                    ...
                                    <th className="px-6 py-4 text-center">Estado Logístico / Pago</th>
                                ...
                                {data.map((purchase, i) => (
                                    <tr key={purchase.id || i} className="hover:bg-[#1a1a1a] subtle-transition">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-white font-medium">{purchase.proveedor ? (typeof purchase.proveedor === 'object' ? purchase.proveedor.nombre : `Prov. ${purchase.proveedor}`) : 'Sin Proveedor'}</div>
                                        ...
                                        <td className="px-6 py-4 font-bold text-right">
                                            <div className="text-[#10b981]">${purchase.total || '0.00'}</div>
                                            {(purchase.saldo_pendiente > 0) && (
                                                <div className="text-[#ef4444] text-xs">Saldo: ${purchase.saldo_pendiente}</div>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <div className="flex flex-col items-center gap-1">
                                                <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wide border 
                                                    ...
                                                    L: {typeof purchase.estado_compra === 'object' ? purchase.estado_compra.nombre : (purchase.estado_compra || 'Pendiente')}
                                                </span>
                                                <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wide border 
                                                    ${purchase.estado_pago === 1 || purchase.estado_pago?.nombre === 'Pagado' ? 'border-[#10b981] text-[#10b981]'
                                                        : purchase.estado_pago === 2 || purchase.estado_pago?.nombre === 'Parcial' ? 'border-[#3b82f6] text-[#3b82f6]'
                                                            : 'border-[#eab308] text-[#eab308]'}`}>
                                                    P: {typeof purchase.estado_pago === 'object' ? purchase.estado_pago.nombre : (purchase.estado_pago || 'Pendiente')}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-center">
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-[#a1a1aa] mb-1">Estado de Progreso</label>
                            <select
                                name="estado_compra"
                                required
                    ...
                    <div className="border-t border-[#27272a] pt-6">
                                    ...
                                    {cartItems.length > 0 ? (
                                        cartItems.map((item, idx) => (
                                            <tr key={idx} className="hover:bg-[#27272a] subtle-transition">
                                                ...
                                                <td className="px-4 py-3 text-right text-[#a1a1aa]">${item.precio_unitario}</td>
                        ...
                        <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-3xl font-bold text-[#10b981]">${cartTotal.toFixed(2)}</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#27272a]">
                                ...
                                <p className="text-white font-medium">
                                    {selectedOrder.proveedor ? (typeof selectedOrder.proveedor === 'object' ? selectedOrder.proveedor.nombre : `Prov. ${selectedOrder.proveedor}`) : 'General'}
                                </p>
                            </div>
                            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#27272a]">
                            ...
                            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#27272a]">
                                ...
                                <p className="text-[#10b981] font-bold text-lg">${selectedOrder.total}</p>
                            </div>
                            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#27272a]">
                                ...
                                <p className={`font-bold text-lg ${selectedOrder.saldo_pendiente > 0 ? 'text-[#ef4444]' : 'text-white'}`}>
                                    ${selectedOrder.saldo_pendiente || '0.00'}
                                </p>
                            </div>
                        </div>
                                        ...
                                        {selectedOrder.items && selectedOrder.items.length > 0 ? (
                                            selectedOrder.items.map((item, idx) => (
                                                <tr key={idx}>
                                                    ...
                                                    <td className="px-4 py-3 text-center text-[#a1a1aa]">{item.cantidad}</td>
                                                    <td className="px-4 py-3 text-right text-[#a1a1aa]">${item.precio_unitario || item.precio}</td>
                                                    <td className="px-4 py-3 text-right text-white font-medium">${(item.cantidad * (item.precio_unitario || item.precio)).toFixed(2)}</td>
                                                </tr>
                                            ))
                                        ) : (
                                            <tr>
