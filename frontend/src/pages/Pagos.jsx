...
export default function Pagos() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const [ventasRes, comprasRes, mediosRes, ordVenRes, ordCompRes] = await Promise.all([
                api.get('/api/v1/pagos-de-ventas/'),
                api.get('/api/v1/pagos-de-compras/'),
                api.get('/api/v1/medios-de-pago/'),
                api.get('/api/v1/ordenes-de-venta/'),
                api.get('/api/v1/ordenes-de-compra/')
            ]);
            setDataVentas(ventasRes.data.results || ventasRes.data || []);
            setDataCompras(comprasRes.data.results || comprasRes.data || []);
            ...
            console.error('Error fetching pagos y dependencias:', error);
                            ...
                            <tbody className="divide-y divide-[#27272a]">
                                {activeData.map((item, i) => (
                                    <tr key={i} className="hover:bg-[#1a1a1a] subtle-transition group">
                                        ...
                                        <td className="px-6 py-4 font-bold text-[#10b981] text-right">${item.monto || '0.00'}</td>
                                        <td className="px-6 py-4 text-[#a1a1aa]">
                                            {item.fecha ? new Date(item.fecha).toLocaleString() : '-'}
                                        </td>
                                        <td className="px-6 py-4 text-[#a1a1aa]">
                                            {item.metodo_pago
                                                ? (typeof item.metodo_pago === 'object' ? item.metodo_pago.nombre : item.metodo_pago)
                                                : '-'}
                                        </td>
            ...
            >
                <div className="space-y-4">
                    <p className="text-[#a1a1aa]">
                        ¿Estás seguro de que deseas anular el pago <strong className="text-white">#{deletingItem?.id}</strong> de <strong className="text-[#10b981]">${deletingItem?.monto}</strong>? Esta acción no se puede deshacer y puede afectar el estado de cuenta de la orden respectiva.
                    </p>
                    <div className="pt-4 flex justify-end gap-3 border-t border-[#27272a]">
                        <button
                            type="button"
