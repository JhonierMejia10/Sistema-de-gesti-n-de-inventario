...
export default function OrdenesVenta() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [
                ...
                api.get('/api/v1/estados-de-pago/'),
                api.get('/api/v1/tipos-de-entrega/')
            ]);
            ...
            setEstadosPago(pagoRes.data.results || pagoRes.data || []);
            setTiposEntrega(entregaRes.data.results || entregaRes.data || []);
            ...
            console.error('Error fetching ventas o dependencias:', error);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.cliente || !formData.almacen || !formData.estado_pago || !formData.tipo_entrega) {
            toast.error('Debes seleccionar todos los campos maestros de la orden');
            return;
        }
                                ...
                                {data.map((sale, i) => (
                                    <tr key={sale.id || i} className="hover:bg-[#1a1a1a] subtle-transition">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-[#a1a1aa] text-xs mt-0.5">{sale.fecha_creacion ? new Date(sale.fecha_creacion).toLocaleDateString() : 'Sin fecha'}</div>
                                        </td>
                                        <td className="px-6 py-4 font-bold text-right">
                                            <div className="text-[#10b981]">${sale.total || '0.00'}</div>
                                            {(sale.saldo_pendiente > 0) && (
                                                <div className="text-[#ef4444] text-xs">Saldo: ${sale.saldo_pendiente}</div>
                                            )}
                                        </td>
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-[#a1a1aa] mb-1">Estado del Pago</label>
                            <select
                                name="estado_pago"
                                required
                            ...
                            >
                                ...
                                {estadosPago.map(ep => <option key={ep.id} value={ep.id}>{ep.nombre}</option>)}
                            </select>
                        </div>
                        <div>
                            ...
                            <select
                                ...
                                className="w-full bg-[#1a1a1a] border border-[#27272a] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#10b981] subtle-transition"
                            >
                                <option value="" disabled>-- Seleccione entrega --</option>
                                {tiposEntrega.map(te => <option key={te.id} value={te.id}>{te.nombre}</option>)}
                            </select>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#27272a]">
                                ...
                                <p className="text-white font-medium">{new Date(selectedOrder.fecha_creacion).toLocaleString()}</p>
                            </div>
                            <div className="bg-[#1a1a1a] p-4 rounded-xl border border-[#27272a]">
